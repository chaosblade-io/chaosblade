...
import { perfMark } from "../utils/perf.js";
...
export type Action =
  ...
  | { type: "NODE_MESSAGE"; content: string; node: string; ts?: number }
  | {
...
/**
 * Canonical display order for the stepper strip. ``recovery`` sits
 * last: it is not part of the happy-path inject pipeline and only
 * materialises when the graph routes into the recover nodes (a
 * /recover turn, or a mid-turn failover).
 */
const _PHASE_STEP_ORDER: readonly PhaseId[] = [
  "intent",
  "safety",
  "inject",
  "verify",
...
/**
 * Nodes whose wire ``phase="inject"`` is really planning work —
 * they run BEFORE the confirmation gate and never touch the
 * cluster. The server labels them ``inject`` because they belong to
 * the inject pipeline structurally, and the L4 ``_PHASE_STEP_MAP``
 * independently labels the same nodes ``planning``. Folding them
 * into the ``intent`` display step keeps the strip honest: the
 * ``inject`` step only lights up once the fault is actually being
 * executed (``baseline_capture`` / ``execute_loop``), so a turn
 * rejected at the Layer-2 gate reads ``[✓ ✗ ○ ○]`` rather than
 * falsely showing ``inject ✓``.
 */
const _PLANNING_PHASE_NODES = new Set([
  "preplan_probe",
  "agent_loop",
  "batch_setup",
]);
...
/**
 * Map a wire (phase, node) pair to a display step. Returns ``null``
 * for phases the stepper deliberately ignores — ``"postmortem"`` /
 * ``"save"`` (terminal bookkeeping, per graph.py's own comment) —
 * and for absent phases (older servers, replay events), both of
 * which mean "no stepper update".
 */
function stepForWirePhase(
  ...
  switch (phase) {
    ...
    case "recovery":
      return phase;
    case "inject":
      return _PLANNING_PHASE_NODES.has(node) ? "intent" : "inject";
    default:
      return null;
  }
}
...
/**
 * Advance the live stepper on a mapped step. Pure helper — returns
 * the SAME reference when nothing changed so callers can assign
 * unconditionally without forcing re-renders.
 *
 * Lazy materialisation gate: the strip only appears once the turn
 * is committed to the inject pipeline (``gateOpen`` — the caller's
 * freshly computed ``currentTurnIsInjection``) or a recovery step
 * arrives. Chat / Q&A turns walk ``preplan_probe`` →
 * ``intent_clarification`` → END with the gate closed the whole
 * way, so they never grow pipeline chrome. A recovery-first strip
 * contains just ``[recovery]`` — earlier canonical steps did not
 * run this turn, and seeding them ``completed`` would fabricate
 * progress.
 *
 * Forward-only transitions: same-step re-fires (LangGraph
 * re-invokes the wrapper on interrupt resume, and several nodes
 * share a phase) and replays of earlier phases (v2 stream replay
 * after a multi-layer interrupt) are no-ops, so the strip never
 * regresses.
 */
function advancePhaseStepper(
  current: PhaseStepperState | null,
  step: PhaseId,
  gateOpen: boolean,
): PhaseStepperState | null {
  const now = Date.now();
  if (!current) {
    if (!gateOpen && step !== "recovery") return null;
    if (step === "recovery") {
      return {
        steps: [{ id: "recovery", status: "in_progress", startedAt: now }],
      };
    }
    // Seed the canonical prefix: steps BEFORE the current one are
    // marked completed (graph order guarantees they ran to reach
    // here), the current one goes in_progress, later ones pending.
    // ``recovery`` stays off the strip until it actually fires.
    // Rounded-up seed steps get NO timestamps — we never observed
    // them run, so an eta there would be fabricated. Only the
    // in_progress step gets a real startedAt.
    const idx = _PHASE_STEP_ORDER.indexOf(step);
    const steps: PhaseStep[] = _PHASE_STEP_ORDER.filter(
      (id) => id !== "recovery",
    ).map((id, i): PhaseStep => ({
      id,
      status:
        i < idx ? "completed" : i === idx ? "in_progress" : "pending",
      startedAt: i === idx ? now : undefined,
    }));
    return { steps };
  }
  const curIdx = current.steps.findIndex((s) => s.status === "in_progress");
  const nextIdx = current.steps.findIndex((s) => s.id === step);
  if (nextIdx === -1) {
    // Step not on the strip yet — recovery arriving mid-inject-turn.
    // Complete whatever is active and append it.
    const steps = current.steps.map((s): PhaseStep =>
      s.status === "in_progress"
        ? { ...s, status: "completed", completedAt: now }
        : s,
    );
    return {
      steps: [...steps, { id: step, status: "in_progress", startedAt: now }],
    };
  }
  if (nextIdx === curIdx) return current;
  if (curIdx !== -1 && nextIdx < curIdx) return current;
  const steps = current.steps.map((s, i): PhaseStep =>
    i === curIdx
      ? { ...s, status: "completed", completedAt: now }
      : i === nextIdx
        ? { ...s, status: "in_progress", startedAt: s.startedAt ?? now }
        : s,
  );
  return { steps };
}
...
/**
 * Produce the committed snapshot of a strip at turn end. Success
 * (``failed=false``) rounds every step up to ``completed`` — the
 * turn reached END, so a step left pending would read as broken.
 * Failure flips the active step to ``failed`` and leaves later
 * steps ``pending``: they never ran, and marking them green would
 * lie about where the pipeline actually broke (the truthful
 * ``[✓ ✓ ✗ ○ ○]`` strip).
 */
function finalisePhaseStepper(
  ...
  return steps.map((s): PhaseStep => {
    ...
    return s.status === "in_progress"
      ? { ...s, status: "failed", completedAt: now }
      : s;
  });
}
  ...
  switch (action.type) {
    ...
    case "TURN_STARTED": {
      ...
      return {
        ...
        // Defensive: the previous turn's strip was finalised into
        // history by commitPending; a turn that died between
        // NODE_STARTED and TURN_DONE (stream drop) must not leak
        // its stale strip into the next turn.
        currentPhaseStepper: null,
        // DAG panel (design §7.2): the last turn's lit/done nodes must
        // not leak into the next turn's skeleton.
        activeNodes: [],
        visitedNodes: [],
        seenToolCallIds: [],
      };
    ...
    case "NODE_STARTED": {
      ...
      // Phase stepper advance. ``step`` is null for phases the
      // strip ignores (postmortem / save / absent on old servers);
      // ``advancePhaseStepper`` returns the same reference on
      // no-ops so the strip doesn't churn re-renders on the many
      // same-phase node transitions within a phase block.
      const step = stepForWirePhase(action.phase, action.node);
      const nextStepper = step
        ? advancePhaseStepper(state.currentPhaseStepper, step, nextIsInjection)
        : state.currentPhaseStepper;

      // DAG panel tracking (design §7.2): dedupe on entry so a loop
      // re-entering a node keeps one visit record and one lit slot.
      const nextActive = state.activeNodes.includes(action.node)
        ? state.activeNodes
        : [...state.activeNodes, action.node];
      const nextVisited = state.visitedNodes.includes(action.node)
        ? state.visitedNodes
        : [...state.visitedNodes, action.node];
    ...
    case "NODE_ENDED":
      // A no-op for every consumer except the DAG panel: the node
      // leaves the lit set but keeps its visit record (renders done).
      if (!state.activeNodes.includes(action.node)) return state;
      return {
        ...state,
    ...
    case "SESSION_INITIALIZED":
      ...
      return {
        ...
        // DAG panel: web's resetSession (sidebar "new session") re-uses
        // this action via dispatchSessionContext — without the reset,
        // the fresh session would display the PREVIOUS session's
        // lit/done graph. Mid-turn new-session is blocked by the
        // sidebar's ``streamState !== "idle"`` gate, so no live node
        // can be lit here. Boot is a no-op (both lists already empty).
        activeNodes: [],
        visitedNodes: [],
        // Session-boundary reset (web session switch / new session):
        // the in-flight-task pointer and the live phase stepper belong
        // to the OLD session — carrying them over would flash the
        // previous session's drill in the fresh session's right rail.
        // RESULT_RECEIVED leaves ``taskId`` set after TURN_DONE (the
        // ``/recover latest`` pointer), and a turn killed before its
        // closing events leaves the stepper materialised — both must
    ...
    case "HISTORY_CLEARED": {
      ...
      // empty items list. The /clear handler also invokes the
      // host-injected ``ctx.clearScreen`` (TUI: ANSI viewport clear)
...
function commitPending(
  ...
  // dropping the buffer.
  state = commitThinking(state);
  // DAG panel: a turn boundary means NOTHING is executing anymore.
  // The happy path never reaches here with a lit node (every
  // NODE_STARTED pairs a NODE_ENDED), but a turn that dies mid-node
  // — Esc, stream drop, protocol error (TURN_ABORTED / TURN_TRANSITION)
  // — never receives that final NODE_ENDED and would otherwise leave
  // the node pulsing "running" forever. Same leak class as the
  // currentPhaseStepper defensive reset, one boundary earlier.
  // ``visitedNodes`` deliberately SURVIVES: the finished turn leaves
  // an all-done graph until the next TURN_STARTED.
  if (state.activeNodes.length > 0) {
    state = { ...state, activeNodes: [] };
  }
  ...
  // Materialise the live phase-stepper, finalised, at the very END
  // of pending — strictly AFTER the optional ⚡ turn-usage line so
  // the strip lands directly above the next InputPrompt in
  // scrollback (see the long comment at the top of this function
  // for the placement rationale and the three-signal failed gate).
  if (state.currentPhaseStepper) {
    const alloc = nextId(state, "ps");
    const stepperItem: PhaseStepperItem = {
      kind: "phase_stepper",
      id: alloc.id,
      steps: finalisePhaseStepper(state.currentPhaseStepper.steps, failed),
    };
    state = {
      ...state,
      pending: [...state.pending, stepperItem],
      nextItemId: alloc.counter,
      currentPhaseStepper: null,
    };
  }
