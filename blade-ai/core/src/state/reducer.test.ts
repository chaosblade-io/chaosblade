...
import { initialAppState, type AppState, type HistoryItem } from "./types.js";
...
describe("reducer / locator allocation", () => {
  ...
  it("assigns E1 to the first RESULT_RECEIVED + captures lastTurnInput", () => {
    ...
    if (e1 && e1.kind === "result") {
      ...
      // phase-14: the result envelope's UID key is ``experiment_uid``
      // (renamed off ``blade_uid`` in phase-9; the read aligned in
      // phase-14) — pin the extraction so a stale-key regression fails
      // here instead of rendering an empty UID row.
      expect(e1.experimentUid).toBe("uid-1");
    }
...
describe("NODE_MESSAGE commit shape", () => {
  ...
  it("carries the receipt timestamp through to the log item", () => {
    // The web rail timeline derives relative timestamps from item.ts.
    const s = fold([
      { type: "NODE_MESSAGE", content: "x", node: "execute", ts: 1_700_000_000_000 },
    ]);
    expect(s.history[0]).toMatchObject({ kind: "log", ts: 1_700_000_000_000 });
  });
});

describe("reducer / phase stepper", () => {
  /** Unwrap the committed strip from history, or fail the test. */
  const stepperFromHistory = (s: AppState) => {
    const item = s.history.find((i) => i.kind === "phase_stepper");
    expect(item).toBeDefined();
    return item as Extract<HistoryItem, { kind: "phase_stepper" }>;
  };

  it("chat turns never grow a strip (gate stays closed)", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "hello" },
      { type: "NODE_STARTED", node: "preplan_probe", phase: "inject" },
      { type: "NODE_STARTED", node: "intent_clarification", phase: "intent" },
      { type: "TURN_DONE" },
    ]);
    expect(s.currentPhaseStepper).toBeNull();
    expect(s.history.some((i) => i.kind === "phase_stepper")).toBe(false);
  });

  it("happy path: seeds at the Layer-1 gate, advances, finalises all-green", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "preplan_probe", phase: "inject" },
      { type: "NODE_STARTED", node: "intent_clarification", phase: "intent" },
      { type: "NODE_STARTED", node: "agent_loop", phase: "inject" },
      // Gate opens here — intent_confirm is the first inject-exclusive node.
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "safety_check", phase: "safety" },
      { type: "NODE_STARTED", node: "confirmation_gate", phase: "safety" },
      ...
      { type: "NODE_STARTED", node: "finalize_verification", phase: "verify" },
      // Terminal bookkeeping phase — deliberately ignored.
      { type: "NODE_STARTED", node: "terminal_reports", phase: "postmortem" },
      { type: "TURN_DONE" },
    ]);
    const item = stepperFromHistory(s);
    expect(item.steps).toMatchObject([
      { id: "intent", status: "completed" },
      { id: "safety", status: "completed" },
      { id: "inject", status: "completed" },
      { id: "verify", status: "completed" },
    ]);
    expect(s.currentPhaseStepper).toBeNull();
  });

  it("live strip seeds with the canonical prefix completed at gate-open", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "preplan_probe", phase: "inject" },
      { type: "NODE_STARTED", node: "intent_clarification", phase: "intent" },
    ...
    expect(s.currentPhaseStepper?.steps).toMatchObject([
      { id: "intent", status: "completed" },
      { id: "safety", status: "in_progress" },
      { id: "inject", status: "pending" },
      { id: "verify", status: "pending" },
    ]);
  });

  it("Layer-2 reject finalises failed: active step ✗, later steps stay pending", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "safety_check", phase: "safety" },
      { type: "NODE_STARTED", node: "confirmation_gate", phase: "safety" },
      { type: "CONFIRM_USER_DECIDED", taskId: "t1", answer: "rejected" },
      { type: "TURN_DONE" },
    ]);
    const item = stepperFromHistory(s);
    expect(item.steps).toMatchObject([
  ...
  it("TURN_ABORTED mid-inject marks the active step failed", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "baseline_capture", phase: "inject" },
      { type: "TURN_ABORTED", reason: "user pressed Esc" },
    ]);
    const item = stepperFromHistory(s);
    expect(item.steps).toMatchObject([
      { id: "intent", status: "completed" },
      { id: "safety", status: "completed" },
      { id: "inject", status: "failed" },
      { id: "verify", status: "pending" },
    ]);
  });

  it("recover turn seeds a recovery-only strip", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "/recover latest" },
      { type: "NODE_STARTED", node: "recover_verifier_loop", phase: "recovery" },
      { type: "NODE_STARTED", node: "finalize_recover_verification", phase: "recovery" },
      { type: "TURN_DONE" },
    ]);
    const item = stepperFromHistory(s);
    expect(item.steps).toMatchObject([{ id: "recovery", status: "completed" }]);
  });

  it("stamps startedAt/completedAt so the web rail can show per-step durations", () => {
    // Mock v2 phase panel eta: the reducer is the ONLY place that
    // observes step transitions, so it stamps them. Seed-rounded
    // completions stay untimestamped (never observed running).
    vi.useFakeTimers();
    try {
      ...
      const seeded = s.currentPhaseStepper?.steps ?? [];
      expect(seeded.find((st) => st.id === "safety")).toMatchObject({
        status: "in_progress",
        startedAt: 1_700_000_000_000,
      });
      // Seed-rounded intent step: no timestamps — never observed.
      expect(seeded.find((st) => st.id === "intent")?.startedAt).toBeUndefined();
      expect(seeded.find((st) => st.id === "intent")?.completedAt).toBeUndefined();
      ...
      const advanced = s.currentPhaseStepper?.steps ?? [];
      expect(advanced.find((st) => st.id === "safety")).toMatchObject({
        status: "completed",
        startedAt: 1_700_000_000_000,
        completedAt: 1_700_000_006_000,
      });
      expect(advanced.find((st) => st.id === "inject")?.startedAt).toBe(
        1_700_000_006_000,
      );

      // Finalise stamps completedAt on the still-active step.
      vi.setSystemTime(1_700_000_012_000);
      s = reducer(s, { type: "TURN_DONE" });
      const item = stepperFromHistory(s);
      expect(item.steps.find((st) => st.id === "inject")).toMatchObject({
        status: "completed",
  ...
  it("old servers without phase never grow a strip", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm" },
      { type: "NODE_STARTED", node: "execute_loop" },
      { type: "TURN_DONE" },
    ]);
    expect(s.currentPhaseStepper).toBeNull();
    expect(s.history.some((i) => i.kind === "phase_stepper")).toBe(false);
  ...
  it("same-phase re-fires (interrupt resume duplicates) are no-ops", () => {
    const base = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "baseline_capture", phase: "inject" },
    ]);
    const before = base.currentPhaseStepper;
    // Resume re-invokes the wrapped node → duplicate phase_started.
    const after = reducer(base, {
      type: "NODE_STARTED",
      node: "baseline_capture",
      phase: "inject",
    });
    expect(after.currentPhaseStepper).toBe(before);
    expect(after.currentPhaseStepper?.steps[2]?.status).toBe("in_progress");
  });

  it("replay of an earlier phase never regresses the strip", () => {
    const base = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "baseline_capture", phase: "inject" },
    ]);
    // LangGraph v2 replay after resume re-emits safety-phase node starts.
    const after = reducer(base, {
      type: "NODE_STARTED",
      node: "safety_check",
      phase: "safety",
    });
    expect(after.currentPhaseStepper).toBe(base.currentPhaseStepper);
    expect(
      after.currentPhaseStepper?.steps.find((s) => s.id === "inject")?.status,
    ).toBe("in_progress");
  });

  it("replan loops back through agent_loop without regressing inject", () => {
    const base = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "baseline_capture", phase: "inject" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      // Replan: graph loops back to agent_loop (planning — remapped
      // wire phase "inject" → intent step), then re-enters execute.
      { type: "NODE_STARTED", node: "agent_loop", phase: "inject" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
    ]);
    const steps = base.currentPhaseStepper?.steps ?? [];
    expect(steps.find((s) => s.id === "inject")?.status).toBe("in_progress");
    expect(steps.find((s) => s.id === "intent")?.status).toBe("completed");
  });

  it("recovery arriving mid-inject-turn appends to the existing strip", () => {
    const base = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "NODE_STARTED", node: "baseline_capture", phase: "inject" },
    ...
    expect(base.currentPhaseStepper?.steps).toMatchObject([
      { id: "intent", status: "completed" },
      { id: "safety", status: "completed" },
      { id: "inject", status: "completed" },
      { id: "verify", status: "completed" },
      { id: "recovery", status: "in_progress" },
    ]);
  });

  it("TURN_TRANSITION (supersede on feedback) finalises the strip into history", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "intent_confirm", phase: "safety" },
      { type: "CONFIRM_USER_DECIDED", taskId: "t1", answer: "rejected", feedback: "use 50%" },
      { type: "TURN_TRANSITION" },
    ]);
    const item = stepperFromHistory(s);
    expect(item.steps.find((st) => st.id === "safety")?.status).toBe("failed");
    expect(s.currentPhaseStepper).toBeNull();
    // …and the follow-up turn starts clean.
    const s2 = reducer(s, { type: "TURN_STARTED", input: "use 50%" });
    expect(s2.currentPhaseStepper).toBeNull();
  });
});

describe("reducer / DAG node tracking", () => {
  // The web DAG panel (design §7.2) reads activeNodes (lit/pulsing) and
  // visitedNodes (done) — both fed exclusively by node_start/node_end
  // events and reset per turn.

  it("tracks active and visited nodes across start/end events", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "agent_loop", phase: "inject" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      { type: "NODE_ENDED", node: "agent_loop" },
    ]);
    expect(s.activeNodes).toEqual(["execute_loop"]);
    expect(s.visitedNodes).toEqual(["agent_loop", "execute_loop"]);
  });

  it("dedupes loop re-entries — one visit record, one lit slot", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      { type: "NODE_ENDED", node: "execute_loop" },
      // A tools round-trip re-enters the same loop node.
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
    ]);
    expect(s.activeNodes).toEqual(["execute_loop"]);
    expect(s.visitedNodes).toEqual(["execute_loop"]);
  });

  it("TURN_STARTED resets both lists so turns never leak", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "agent_loop", phase: "inject" },
      { type: "NODE_ENDED", node: "agent_loop" },
    ...
    expect(s.activeNodes).toEqual([]);
    expect(s.visitedNodes).toEqual([]);
  });

  it("NODE_ENDED for a non-active node is a same-reference no-op", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "agent_loop", phase: "inject" },
    ]);
    // Stray end events (e.g. an unmatched pair after a stream gap)
    // must not churn the state — the DAG panel subscribes to it.
    expect(reducer(s, { type: "NODE_ENDED", node: "verifier_loop" })).toBe(s);
  });

  it("TURN_ABORTED mid-node clears the lit set but keeps the visit record", () => {
    // Esc / stream drop between NODE_STARTED and its NODE_ENDED: the
    // closing event never arrives, so without a turn-commit clear the
    // node would pulse "running" forever. The visit record SURVIVES —
    // the node genuinely ran — and the panel renders it done.
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      { type: "TURN_ABORTED", reason: "user aborted" },
    ]);
    expect(s.activeNodes).toEqual([]);
    expect(s.visitedNodes).toEqual(["execute_loop"]);
  });

  it("TURN_DONE clears the lit set the same way (defensive — pairs normally close first)", () => {
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "verifier_loop", phase: "verify" },
      { type: "TURN_DONE" },
    ...
    expect(s.activeNodes).toEqual([]);
    expect(s.visitedNodes).toEqual(["verifier_loop"]);
  });

  it("SESSION_INITIALIZED clears both lists — a new session must not show the old one's graph", () => {
    // Web's resetSession re-uses this action after swapping session
    // ids; without the reset the fresh session would inherit the
    // previous session's all-done graph.
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      { type: "TURN_DONE" },
    ...
    expect(s.activeNodes).toEqual([]);
    expect(s.visitedNodes).toEqual([]);
    expect(s.session.id).toBe("s-new");
  });

  it("SESSION_INITIALIZED clears taskId — a finished turn's pointer must not leak into the next session", () => {
    // RESULT_RECEIVED leaves ``taskId`` set past TURN_DONE (it backs
    // ``/recover latest``); web's switchSession/resetSession re-use this
    // action at the session boundary, so the pointer must reset or the
    // fresh session's right rail would light up with the OLD drill.
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      { type: "RESULT_RECEIVED", content: "ok", taskId: "T1" },
  ...
  it("SESSION_INITIALIZED clears a mid-turn live stepper (defensive — UI gates mid-turn switches)", () => {
    // The web switch gate (streamState !== "idle") should prevent this
    // sequence outright, but the reducer is the last line: a stepper
    // materialised by NODE_STARTED must not survive a session swap.
    const s = fold([
      { type: "TURN_STARTED", input: "inject cpu" },
      { type: "NODE_STARTED", node: "execute_loop", phase: "inject" },
      {
