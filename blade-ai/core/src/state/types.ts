...
export interface ResultItem {
  ...
  sideEffects?: string[];
  /** Backend-assembled one-line summary of all side-effect detection
   *  results (e.g. "No collateral impact detected (ContainerRestarts: 0, OOMKill: 0, ...)").
   *  Always present when side-effect detection ran; undefined otherwise. */
  sideEffectsSummary?: string;
  /** T6 — Postmortem auto-generated by save_memory after a real
   *  experiment. Absent when postmortem disabled / LLM timed out /
   *  non-injection intent / non-whitelist failure. PostmortemSection
   *  renders ``markdown`` (full body) with a light-weight markdown
   *  renderer; ``summary`` is the 1-2 sentence preview line; ``path``
   *  is the on-disk markdown for sharing / ``blade-ai postmortem`` CLI. */
  postmortem?: {
    path: string;
    markdown: string;
    summary: string;
...
export interface ConfirmPromptItem {
   ...
   *  For plan_builder: raw selection key ("A", "B", free text).
   *  For others: "approved" | "rejected". */
  answer?: string;
...
export interface LogItem {
  ...
  tag?: string;
  /** Receipt time (epoch ms) for ``node_message`` progress lines —
   *  server ``timestamp`` when the wire carries one, client clock
   *  otherwise. Drives the web rail timeline's relative timestamps.
   *  Absent for slash-command logs and pre-field hydrated items. */
  ts?: number;
}
 ...
 *
 * The server wraps every major graph node in ``with_phase_events``
 * (``agent/dispatch.py``) which emits ``node_start`` events carrying a
 * wire phase: ``"intent" | "safety" | "inject" | "verify" |
 * "recovery"`` (plus ``"postmortem"`` / ``"save"`` which the
 * stepper deliberately ignores — terminal bookkeeping, not pipeline
 * progress). The reducer folds those events into a live
 * ``PhaseStepperState`` on ``AppState.currentPhaseStepper``; at
 * TURN_DONE / TURN_ABORTED / TURN_TRANSITION ``commitPending``
 * finalises the strip and appends it to history as a
 * ``PhaseStepperItem`` so scrollback keeps the "where did this turn
 * get to" summary pinned at the bottom of the turn block.
 *
 * Wire-phase → display-step remap: nodes whose ``inject`` phase is
 * really planning work (``preplan_probe`` / ``agent_loop`` /
 * ``batch_setup`` — the L4 ``_PHASE_STEP_MAP`` agrees: it labels
 * them ``planning``) fold into the ``intent`` step, so the
 * ``inject`` step only lights up when the fault is actually being
 * executed (``baseline_capture`` / ``execute_loop``). Without the
 * remap a rejected-at-L2 turn would show ``inject ✓`` even though no
 * fault ever touched the cluster.
 */
export type PhaseId = "intent" | "safety" | "inject" | "verify" | "recovery";
...
export interface PhaseStep {
  id: PhaseId;
  status: PhaseStatus;
  /** Epoch ms when the step went in_progress. Absent on steps the
   *  seed path rounds up to ``completed`` (their timing was never
   *  observed — showing a duration there would be fabricated) and
   *  on steps that never left ``pending``. */
  startedAt?: number;
  /** Epoch ms when the step left in_progress (advance → completed,
   *  or finalise → completed/failed). Together with ``startedAt``
   *  this drives the per-step duration eta in the web rail. */
  completedAt?: number;
}

/** Live strip shape. Exactly one step is ``in_progress`` while the
 *  turn is in flight; the strip materialises lazily (see the reducer
 *  gate) so chat / Q&A turns never grow one. */
export interface PhaseStepperState {
  /** Display steps in canonical order. ``recovery`` is absent from
   *  a normal inject turn's strip and appears only when the graph
   *  actually routes into the recover nodes. */
  steps: PhaseStep[];
}

/** Finalised strip committed to history at turn end. No
 *  ``in_progress`` remains: success rounds every step up to
 *  ``completed``; failure flips the active step to ``failed`` and
 *  leaves later steps ``pending`` (they never ran — marking them
 *  green would lie about where the pipeline broke). */
...
export interface AppState {
  ...
  /**
   * Edge-triggered boolean mirroring ``thoughtBuffer.length > 0``.
   * Subscribers (LoadingIndicator) read THIS instead of subscribing
   * to ``thoughtBuffer`` directly so they only re-render on the
   * 0→N / N→0 thinking-session edges, not on every appended token
   * chunk. ``thoughtBuffer`` is still authoritative for the on-screen
   * "▸ Thought for Ns" collapse — this flag is purely a perf signal
   * for high-frequency subscribers.
   */
  hasActiveThinking: boolean;
  /**
   * Set true after the first ``ThinkingItem`` is committed for the
   * current agent response segment. While true, ``commitThinking``
   * silently discards buffered thinking instead of creating a new
   * ``ThinkingItem`` — this prevents trailing/interleaved thinking
   * blocks caused by Qwen's ``enable_thinking`` mode emitting
   * reasoning chunks mid-content. Reset at boundary events
   * (TOOL_STARTED, CONFIRM_RECEIVED, NODE_STARTED) so genuine
   * multi-phase thinking (think → tool → think → reply) still
   * renders correctly.
   */
  suppressMidContentThinking: boolean;
  /**
  ...
  /**
   * Live phase-stepper strip for the in-flight turn. ``null`` for
   * chat / Q&A turns and on old servers that omit ``phase`` from
   * ``node_start`` events — the strip only materialises once the
   * turn commits to the inject pipeline (``currentTurnIsInjection``
   * flips) or a recovery phase arrives, so plain dialogue never
   * grows pipeline chrome.
   *
   * Deliberately NOT in ``pending``: a live strip there would block
   * the leading-stable flush behind a permanently-unresolved head
   * item (same failure mode as a stuck running tool_group). The
   * hosts pin it above the input area; ``commitPending`` finalises
   * it into a ``PhaseStepperItem`` at turn end.
   */
  currentPhaseStepper: PhaseStepperState | null;
  /**
   * Live LangGraph node tracking for the DAG panel (web design §7.2):
   * node names from ``node_start``/``node_end`` events (the
   * ``with_phase_events`` instrumentation — only the 17 wrapped nodes
   * emit, screener/tool plumbing never appears here).
   *
   * - ``activeNodes`` — currently executing (lit / pulsing in the DAG).
   * - ``visitedNodes`` — entered at least once this turn, in first-entry
   *   order (renders as the done state once no longer active). Loops
   *   re-entering a node keep a single entry; a LangGraph resume
   *   re-invoking a node re-lights it, which mirrors real execution
   *   (dispatch.py: the phase_started duplicate on resume is the node
   *   genuinely re-running, not a replay artifact).
   *
   * Both reset on TURN_STARTED and on SESSION_INITIALIZED (web's
   * new-session swap re-uses that action — a fresh session must not
   * show the previous session's graph). ``activeNodes`` additionally
   * clears at every turn commit (TURN_DONE / TURN_ABORTED /
   * TURN_TRANSITION — commitPending), because a turn that dies
   * mid-node never receives the closing ``node_end`` and must not
   * leave the node lit forever; ``visitedNodes`` survives so the
   * finished turn leaves an all-done graph until the next turn starts.
   * Recordings never carry node events (tui_session_store drops them),
   * so replays leave both lists empty and the panel simply renders its
   * static skeleton.
   */
  activeNodes: string[];
  visitedNodes: string[];
