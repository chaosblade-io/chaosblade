...
export interface SlashCommandContext {
  ...
  /**
   * Host build identity shown on the /doctor card (``tuiVersion``
   * field). Optional so bare-bones harnesses (smoke scripts) can skip
   * it; the card falls back to ``?``.
   */
  hostVersion?: string;
  /**
   * Wipe the host's already-committed output region BEFORE the
   * HISTORY_CLEARED dispatch (TUI: ANSI ``\\x1b[H\\x1b[J`` to clear
   * terminal scrollback; web: unneeded — there is no burn-in'd
   * scrollback, clearing React state is enough). Absent = no-op.
   */
  clearScreen?: () => void;
  /**
   * Persist text to a user-supplied file path (``/recordings export``).
   * The host resolves ``~``, creates parent dirs, and MUST refuse to
   * overwrite: return ``{ saved: false, alreadyExists: true }`` when
   * the target exists. Any other failure should throw — the handler's
   * try/catch renders it via ``recordings.export_failed``.
   * Absent on hosts without a writable filesystem (web).
   */
  saveTextFile?: (
    outPath: string,
    content: string,
...
async function runLocatorShowHandler(
  ...
  const lines = [
    ...
    `  task=${item.taskId}`,
    `  status=${item.status}`,
    item.experimentUid ? `  uid=${item.experimentUid}` : "",
...
async function runLocatorCopyHandler(
  ...
  const payload = {
    ...
    status: item.status,
    experiment_uid: item.experimentUid,
...
/**
 * The viewport-clear ANSI sequence lives with the host that writes it
 * (``tui/src/components/Composer.tsx`` — ``\\x1b[H\\x1b[J``, viewport-only,
 * scrollback preserved; see the rationale comment there). Core only
 * calls the injected ``ctx.clearScreen`` hook from the /clear handler.
 */
  ...
  return [
    ...
    },
    {
      ...
      async handler(ctx) {
        ...
        // start its own append-only stream from scratch. The actual
        // wipe is host-injected: the TUI writes ANSI ``\\x1b[H\\x1b[J``
        // to stdout; hosts without scrollback (web) simply omit it.
        try {
          ctx.clearScreen?.();
        } catch {
          // Best-effort — tests / non-TTY contexts may have a stub stdout.
        ...
        export: {
          ...
          async handler(ctx, args) {
            ...
            try {
              ...
              // The write itself is host-injected (``saveTextFile``)
              // so this module carries no ``node:fs`` reference — the
              // registry must stay importable in a browser bundle. The
              // JSONL serialisation stays here: it's pure data shaping,
              // identical for every host.
              const save = ctx.saveTextFile;
              if (!save) {
                pushLog(ctx, t("recordings.export_unsupported"), "warn");
                return;
              }
              const jsonl =
                events.map((e) => JSON.stringify(e)).join("\n") + "\n";
              const outcome = await save(outPath, jsonl);
              if (!outcome.saved) {
                pushLog(
                  ctx,
                  t("recordings.export_exists", { path: outcome.absPath }),
                  "warn",
                );
                return;
              }
 ...
 *  Exported for the web tasks table (same label authority). */
export function formatFaultType(row: Record<string, unknown>): string {
...
export function formatReviewCard(
  ...
  const experimentUid = (data["experiment_uid"] as string) || "";
  const lines = [
  ...
  if (experimentUid) lines.push(`  ${t("review.uid_label")}: ${experimentUid}`);
  if (durationMs > 0) {
  ...
  // only when present, same discipline as experiment_uid above.
