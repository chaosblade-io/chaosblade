...
2. 对指定 gRPC 方法注入延迟:
   ```bash
   blade create python grpc delay --time <time> --service <service> --method <method> --timeout <duration>
   ```
   - `--time`:延迟毫秒数(**必填**)
...
**注入验证**：
1. **（主证据，必做）** 再次触发该调用,确认耗时比基线增加约 `--time` 毫秒
2. **（诊断，仅当上述效果未出现时执行）**查询实验状态——机制回执为机制代言不为结果代言，实验在册仅证明拦截规则已注册（用于区分「实验未在册」与「在册但应用未命中拦截点」），不构成效果证据:
   ```bash
