...
# 本 case 恢复动作住址 = apiserver 写（逆 patch 解除 cordon（uncordon）），但靶
# 为 Node（cluster-scoped）——装配器 M1 四对象栈仅支持 namespaced 靶
# （Role/RoleBinding 无法授权 cluster-scoped 资源，工具对 Node 靶 fail-closed
# 拒绝），本 case 无装配器主路径：恢复走正文 SOP 路径（宿主机 systemd timer
# 武装 uncordon 自治——kubelet.conf 凭证对 spec.unschedulable 实际放行），
# 属 recovery-carrier spec「降级路径」的结构性形态，恢复闭环不缺失。
recovery_channel: apiserver-write
# 机制写入集立法（write-set approval contract）：本 case 故障机制只需写受害者
# 自身（cordon/drain 写靶 Node 的 spec 与驱逐其上 Pod 属靶域内，名字匹配放
# 行），无跨对象写条目；宿主机 timer 通道（systemd-run）不建 k8s 对象，无立
# 法条目。
---
...
targetRef:                                # 靶（cluster-scoped，无 namespace）
  kind: Node
  name: <node-name>
patches:                                  # 注入域（json-patch 语义；实际经 kubectl cordon/drain 命令执行）
- op: replace
