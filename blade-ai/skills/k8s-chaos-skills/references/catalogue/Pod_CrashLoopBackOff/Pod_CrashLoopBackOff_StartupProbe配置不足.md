...
# 本 case 恢复动作住址 = apiserver 写（逆 patch 还原 startupProbe 配置），路由进 FaultDrill
...
**CR 通道模板**（`recovery_channel: apiserver-write`——恢复动作住址 = apiserver 写：逆 patch 还原 startupProbe 配置；planning 优先路由 FaultDrill CR 通道，CRD 不可装时降级正文 SOP 形态）：
