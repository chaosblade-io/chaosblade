   ...
   spec:
     ...
     resources:
       requests:
         storage: 10Gi
   ```
3. **先捕获基线，再武装恢复**（先捕获 volumes/volumeMounts 数组基线与 maxUnavailable
   原值，恢复按基线整体还原。恢复命令幂等：定时器自动执行，迟到重复执行
   无副作用）。⚠️ **定时器宿主要求（先武装定时恢复，再注入）**：恢复定时器可武装在
   ...
   「Agent 在线保活」形态）。
   ⚠️ **效果采样职责**：无论定时器宿主形态（外部工具 Pod 还是标准件载体），都必须在**注入生效后、恢复执行前**的窗口中段完成一次效果采样并
   ...
   即成为 approved plan 的一部分，执行器会依计划执行。定时器宿主形态下在武装前
   （注入生效确认后）执行一次同款采样即可满足——verify 的 E-decides 裁决依据
   （verifier 采信中段采样）
   ```bash
   # 基线捕获：Agent 读取输出并记录原始数组 JSON（恢复对照基准）
   kubectl get deployment <deployment-name> -n <namespace> \
...
自愈——PVC 未绑定前 scheduler 永不调度），verify **不必等恢复**——定时器武装
+ 注入生效确认（上面 1-5 的观察）后即可进入效果裁决：窗口内观察到「全部目标
Pod Pending + FailedScheduling unbound PVC 事件 + PVC Pending」即效果确证
（EVIDENCE SEMANTICS：效果裁决不等恢复窗口）。恢复由定时器带外完成，恢复确认
（Pod Running + PVC 清理）走演练结束后的带外核验

**注入恢复**：
1. 等待 `<duration>` 到期执行基线还原：定时器宿主形态由定时器自动执行（宿主要求
   见步骤 3）；演练提前结束时由 Agent 主动执行同一条恢复命令
   （幂等，定时器迟到再执行一次无副作用。**数组整体 replace 回基线而非按索引 remove**
