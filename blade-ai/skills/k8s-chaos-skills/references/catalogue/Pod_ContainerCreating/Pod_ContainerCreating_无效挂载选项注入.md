...
# 本 case 恢复动作住址 = apiserver 写（逆 patch 还原 PV mountOptions 至基线），路由进 FaultDrill
...
**CR 通道模板**（`recovery_channel: apiserver-write`——恢复动作住址 = apiserver 写：逆 patch 还原 PV mountOptions 至基线；planning 优先路由 FaultDrill CR 通道，CRD 不可装时降级正文 SOP 形态）：
...
- cluster-scoped 目标（PersistentVolume）：targetRef 不写 namespace；CR 本体落任务主 ns（metadata.namespace 仍必须显式写入）。
