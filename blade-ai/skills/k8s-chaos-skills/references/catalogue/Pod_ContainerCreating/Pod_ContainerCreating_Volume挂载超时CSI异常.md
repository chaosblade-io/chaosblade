...
# 本 case 恢复动作住址 = apiserver 写（逆 patch 移除注入的卷挂载并还原 maxUnavailable），路由进 FaultDrill
...
**CR 通道模板**（`recovery_channel: apiserver-write`——恢复动作住址 = apiserver 写：逆 patch 移除注入的卷挂载并还原 maxUnavailable；planning 优先路由 FaultDrill CR 通道，CRD 不可装时降级正文 SOP 形态）：
...
- 道具 PV/PVC（假盘 volumeHandle / 静态绑定 PVC）为 execute 计划 manifest 步骤（先 PV 后 PVC 分两次 apply——正文多文档禁令立法不变，PVC 走 front matter mechanism_writes 立法条目）；收尾清理步删除（先 PVC 后 PV；PV 卡 Terminating 时 json-patch 移除 finalizers 兜底——正文恢复段）。
...
4. （载体验权后）武装定时器：恢复合三个 REST 动作，**curl 顺序即执行顺序，PVC 先删、PV 后删**（PVC 存在时 PV 删除会被绑定关系阻塞），用紧凑变量形态的逐 curl 变体（变量赋值与引用全部写在 `sh -c` 载荷内）。`<duration>` 需覆盖滚动更新与观察窗口：
   ```bash
...
5. 修改应用 A 的工作负载模板，添加引用该 PVC 的 volume 和 volumeMount
6. （Deployment）等待滚动更新完成，确认所有旧 Pod 已被替换；（StatefulSet）删除目标 Pod 触发重建
7. （仅 Deployment）滚动更新完成后，立即还原 maxUnavailable 为原始值（maxUnavailable 只是使滚动更新完成的手段，不是故障本身，不应泄漏到恢复阶段）
...
**注入恢复**：
1. 等待 `<duration>` 到期后武装的定时器自动移除注入的 volumes/volumeMounts 并清理 PV/PVC；如需提前恢复，Agent 直接执行下列第 2–6 步恢复命令（幂等，定时器迟到再执行一次无副作用）
...
5. 清理测试 PVC：`kubectl delete pvc archive-vol-claim -n <namespace>`（**先于 PV**——PVC 存在时 PV 删除被绑定关系阻塞）
6. 清理测试 PV：`kubectl delete pv archive-vol-chaos`；**若 PV 卡 Terminating**（CSI external-attacher 的 finalizer 因假盘 detach 无法正常释放），json patch 移除 finalizers 是标准兑底：`kubectl patch pv archive-vol-chaos --type='json' -p '[{"op":"remove","path":"/metadata/finalizers"}]'`（此时底层盘本就不存在，无真实数据风险）
