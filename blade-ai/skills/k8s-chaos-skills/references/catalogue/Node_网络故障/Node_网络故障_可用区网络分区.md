...
3. 确认 ChaosBlade Operator 已部署（DaemonSet 通道）——若 operator 长期不可用（如 ImagePullBackOff 挂起，多 case 复证判死），手段1 直接判死，走手段2 kubectl-native 路径
4. 确认目标 AZ 内节点数量，评估影响面
5. **识别禁止注入的排除节点 `EXCLUDED_NODES`**（承载控制面与命令下发基础设施的节点，**必须从注入目标中剔除、禁止注入**——一旦被分区，集群控制/带内下发与恢复通道即失联，演练将不可控、不可恢复；它们是刻意留存的观测与恢复支点）：
   - **API Server / 控制面节点**（无论用哪种通道都禁止注入）。落在目标 AZ 内时从注入列表剔除：
     ```bash
     kubectl get nodes -l node-role.kubernetes.io/control-plane -o wide
     ```
   - **命令下发基础设施所在节点**：若命令经集群内的下发代理 Pod 转发到目标节点，该 Pod 所在节点是带内下发咽喉，禁止注入。按实际使用的下发组件定位其落点 Pod，取 NODE 列即为排除节点：
     ```bash
     kubectl get pods -n <下发组件ns> -l <下发组件标签> -o wide
     ```
...
2. 对目标 AZ **除排除节点外**的全部节点注入网络屏蔽（`EXCLUDED_NODES` 不在注入范围内）：
   ...
   **方式一：DaemonSet 通道**（⚠️ 注入后无法通过 K8s API 恢复，必须指定 `--timeout`）
   ...
   ```bash
   # 批次 1..N：目标 AZ 节点去掉 EXCLUDED_NODES 后的列表。每批 --names 长度须控制在命令下发通道的内联上限内
   #（以实际通道上限为准），超出则拆成多批顺序提交。
   blade create k8s node-network drop \
     --names <node1>,<node2>,...,<nodeK> \
     --timeout <duration>

   ```
   ⚠️ 严禁把 `EXCLUDED_NODES`（API Server/控制面节点、命令下发基础设施所在节点）纳入任何一批。

3. 记录返回的 experiment_uid，用于后续恢复

> ⚠️ **注入范围与批量约束（排除节点禁止注入、其余可分批）**：目标 AZ 节点须先剔除 `EXCLUDED_NODES`，仅对剩余节点注入，必须同时满足两条不变量：
>
> 1. **排除节点禁止注入**：把"资源准备"识别出的排除节点 `EXCLUDED_NODES`（API Server/控制面节点，以及命令下发基础设施所在节点）**从注入列表中彻底剔除，任何批次都不得包含它们**——一旦它们被分区，集群控制面/带内下发与恢复通道即失联，演练将不可控、不可恢复。它们是刻意留存的观测与恢复支点，不是"漏注入"，报告中应说明其为"按安全策略排除"。
> 2. **其余节点可分批并发**：非排除节点不承载控制/下发通道，**可批量同时 DROP，无需逐个串行**。唯一约束是**每批 `--names` 长度须控制在命令下发通道的内联上限内**（以实际通道上限为准），超出则拆成多批顺序提交。若某一批整体失败（如命令超长被拒、下发通道抖动），**可对该批内失败的节点逐个单独重试**，成功一个记一个，不因整批失败而放弃剩余节点。
>
...
> - **禁止从局部推断整体**：成功口径以**已注入的目标节点（目标 AZ 去掉 `EXCLUDED_NODES` 后的集合）**为分母——只有当集群侧快照覆盖到全部已注入节点且均异常，才可称"全部成功"；仍有未覆盖/仍 Ready 的已注入节点时，只能报"部分成功 N/已注入总数"。`EXCLUDED_NODES`（API Server/命令下发基础设施节点）本就不在注入范围内、预期保持 Ready，**不得**计入失败或部分，也**不得**输出"集群整体不可用/全部节点已注入"。

1. 检查目标 AZ 节点状态（预期全部变为 NotReady，**这是首要且可靠的判据**）：
   ```bash
   kubectl get nodes -l topology.kubernetes.io/zone=<az-name>
   ```
   NotReady 需 kubelet 心跳停止满 node-monitor-grace-period（默认 40s）后才陆续出现——注入后 40s 内该 AZ 节点仍 Ready 是预期中间态，不算「仍 Ready 的已注入节点」，勿据此报部分成功；成功口径的集群侧快照应在注入满 40s 后获取。
2. 检查工作负载迁移情况：
   ```bash
...
注入命令：
```bash
# 1. 获取目标 AZ 所有节点；剔除排除节点 EXCLUDED_NODES（见"资源准备"：API Server/控制面节点、命令下发基础设施所在节点）
...
# 注意：EXCLUDED_NODES（API Server/控制面节点、命令下发基础设施所在节点）禁止注入，
#      保留为观测与 SSH 恢复支点，不纳入上面的循环。
```
方案A 倒计时从武装时刻起算：systemd-run 武装与 DROP 注入在同一载荷内 && 串联原子紧邻（先武装是防御性顺序）；注入后 exec 通道已断，武装后发生任何修复需重武装时必须走带外 SSH：`ssh root@<node-ip> 'systemctl stop blade-restore-az.timer 2>/dev/null; systemctl stop blade-restore-az.service 2>/dev/null; systemctl reset-failed blade-restore-az.service'` 再重跑 systemd-run 武装命令（对可用区内每个受影响节点逐一执行）（见 SKILL.md 安全红线「故障窗口完整」）
...
- 建议超时设置 60-180 秒，避免长时间网络分区导致大量 Pod 重新调度后的状态混乱；窗口短于节点默认 NoExecute 容忍（not-ready/unreachable 300s）时，多数业务 Pod 只经历瞬时 NotReady 而不触发驱逐——恢复验证不应把"零驱逐"误判为注入失败（驱逐是否发生取决于窗口长度与各负载自定义容忍，与注入成功与否无关）
- 集群存在外部节点自愈通道（云厂商 NodeRepair 类，NotReady 后分钟级介入）时，**timer fire 时刻必须设计为先于自愈介入**（如 120s fire vs 217-292s 介入下界——AZ 级多节点齐断时介入规模未知，fire 先行是唯一可控规避）；fire 被外部通道抢跑时注入效果时间窗不可控，报告须标注
- 生产环境建议安装 ChaosBlade 后使用 DaemonSet 通道 + `--timeout` 执行 AZ 级网络分区（operator 可用时优先）
