...
class TestFormatRelativeTime:
    """format_relative_time() renders compact relative labels."""

    _NOW = datetime(2026, 6, 24, 10, 0, tzinfo=BEIJING_TZ)

    def test_today(self):
        ts = "2026-06-24T09:10:00+08:00"
        assert format_relative_time(ts, now=self._NOW) == "today 09:10"

    def test_yesterday(self):
        ts = "2026-06-23T15:02:00+08:00"
        assert format_relative_time(ts, now=self._NOW) == "yesterday 15:02"

    def test_n_days_ago(self):
        ts = "2026-06-21T14:23:00+08:00"
        assert format_relative_time(ts, now=self._NOW) == "3d ago 14:23"
    ...
    def test_calendar_date_bucket_not_hours(self):
        """A 23:30 injection viewed at 01:00 next morning reads as yesterday."""
        now = datetime(2026, 6, 24, 1, 0, tzinfo=BEIJING_TZ)
        ts = "2026-06-23T23:30:00+08:00"
        assert format_relative_time(ts, now=now) == "yesterday 23:30"
    ...
    def test_k8s_utc_timestamp_converted_to_beijing(self):
        ...
        assert format_relative_time(ts, now=self._NOW) == "yesterday 16:00"
