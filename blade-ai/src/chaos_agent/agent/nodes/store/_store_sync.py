...
async def sync_to_store(state: dict, updated_fields: dict) -> None:
    ...
    Only a real ``task-`` identity is synced (see
    ``persistence.task_identity``).  The intent graph runs the same
    nodes during pure dialogue, where the state carries no task —
    syncing then would fabricate a "ghost" experiment that later shows
    up in the recover flow with no injection state to roll back.
    """
