...
from chaos_agent.agent.state import AgentState, terminal_task_state
...
async def batch_next(state: AgentState) -> dict:
    ...
    # Terminal record for this fault: batch_next runs after save_memory /
    # reject, so an unverified run is finished-and-unverified, not in
    # progress. Shares the single-injection rule via terminal_task_state so
    # the two cannot report the same situation differently.
    task_state = terminal_task_state(dict(state))

    from chaos_agent.agent.spec.fault_spec import legacy_target_dict
