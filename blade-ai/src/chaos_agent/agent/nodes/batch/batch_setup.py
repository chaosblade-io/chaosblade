...
async def batch_setup(state: AgentState) -> dict:
    ...
    # Intent clarification persists batch entries as FaultSpec dictionaries.
    # Accept the older submit-tool shape only for externally constructed input.
    spec = FaultSpec.from_dict(current) if isinstance(current, dict) else None
    if spec is None:
        spec = FaultSpec(
