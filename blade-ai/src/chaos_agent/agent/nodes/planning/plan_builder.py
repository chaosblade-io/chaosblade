...
1. Discovers targets (kubectl_read) — routes to ToolNode
...
- tool_calls (kubectl_read etc.) → RETURN to graph (ToolNode executes, then loops back)
...
from chaos_agent.agent.planning.mode import (
    PLANNING_MODE_EXPERT,
    PLANNING_MODE_GUIDED,
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "execution_order": {
                "type": "string",
                "enum": ["serial"],
                "description": "Only serial execution is currently implemented",
            },
            "interval_seconds": {
                "type": "integer",
...
def make_plan_builder(llm=None, tools: list = None, hook=None, registry=None):
    ...
    Args:
        ...
        tools: Phase1 tools (kubectl_read, activate_skill, read_skill_resource).
    ...
    async def plan_builder(state: AgentState) -> dict:
        ...
        if hook:
            ...
            # can replace the message list, and the copy taken above predates it.
            messages = list(state.get("messages", []))

        existing_spec = read_fault_spec(state)
        planning_mode = resolve_planning_mode(state, existing_spec)

        capability_context = build_capability_context(state, "plan", tools or [])
        ...
        function_schemas = [SUBMIT_PLAN_TOOL]
        if planning_mode == PLANNING_MODE_GUIDED:
            function_schemas.append(PRESENT_OPTIONS_TOOL)
        llm_bound = llm.bind_tools(function_schemas + visible_tools)

        # Guided resource discovery is an orchestration responsibility, not a
        # prompt command.  Send one bounded, read-only prefetch before the LLM
        # ranks options; expert mode retains control of when discovery matters.
        guided_prefetch = build_guided_prefetch(existing_spec, capability_context)
        if (
            planning_mode == PLANNING_MODE_GUIDED
            and not state.get("plan_builder_prefetch_done")
            and guided_prefetch is not None
        ):
            return merge_hook_updates({
                "messages": [AIMessage(
                    content="",
                    tool_calls=[{
                        "name": guided_prefetch.tool_name,
                        "id": f"pb-prefetch-{uuid.uuid4().hex[:8]}",
                        "args": guided_prefetch.args,
                    }],
                )],
                "planning_mode": planning_mode,
                "plan_builder_prefetch_done": True,
                "plan_builder_round": plan_builder_round + 1,
            }, hook_updates)
        ...
        while plan_builder_round + rounds_this_invoke < MAX_PLAN_BUILDER_ROUNDS:
            ...
            try:
                ...
                # message size and ranks the user's own request no higher than a
                # routine tool result — in a real drill the single HumanMessage
                # sits at index 0 and is the first thing such a window drops.
                # Bounding context belongs to the compaction hook invoked above.
                # ``accumulated`` stays appended: it holds this invocation's
                # interrupt/resume turns, which are NOT yet in ``messages``.
                response = await llm_bound.ainvoke(
                    [system_msg] + messages + accumulated
                )
            ...
            if options_args:
                if planning_mode == PLANNING_MODE_EXPERT:
                    # The UI option tool is deliberately absent from expert
            ...
            answer = interrupt({
                "type": "expert_confirmation" if planning_mode == PLANNING_MODE_EXPERT else "plan_selection",
    ...
    async def plan_builder_with_probe_cleanup(state: AgentState) -> dict:
        """Wrap plan_builder to clean up capability-probe debug pods on exit.

        plan_builder is a terminal path (``plan_builder -> END / batch_setup``)
        that never reaches ``extract_planning_metadata``'s cleanup. When a turn
        ENDS the loop (last message has no tool_calls, so
        ``should_continue_plan_builder`` routes to END) we remove any ephemeral
        ``kubectl_read debug`` probe pods created during this /plan dry-run so
        they do not leak. Turns that return real tool_calls (loop continues via
        plan_builder_tools) skip cleanup. ``interrupt()`` pauses propagate
        unchanged (they never reach this post-return check).
        """
        result = await plan_builder(state)
        msgs = result.get("messages") or []
        last = msgs[-1] if msgs else None
        has_tool_calls = bool(last is not None and getattr(last, "tool_calls", None))
    ...
    return plan_builder_with_probe_cleanup
...
def _filter_internal_from_response(response) -> AIMessage:
    ...
    Real tools (kubectl_read etc.) preserved for ToolNode routing.
    """
    filtered_tc = [
