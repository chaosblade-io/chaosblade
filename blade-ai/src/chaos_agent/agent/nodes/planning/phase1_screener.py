...
Why this exists despite the earlier layers (A-E):
  ...
  - Layer A removes the full ``kubectl`` from Phase 1 tool surface
    (replaced by ``kubectl_read``). This kills the documented bypass
...
async def phase1_screener(state: AgentState) -> dict[str, Any]:
    ...
    # Truncated response: agent_loop already neutralised the batch — parseable
    ...
    # precondition. A stale flag would otherwise divert a FRESH batch, and the
    # loop would spin without those calls ever running.
    if state.get("truncated_tool_calls"):
        pending = isinstance(last, AIMessage) and getattr(last, "tool_calls", None)
        if not pending:
            return {
        ...
        logger.warning(
            "phase1_screener: stale truncated_tool_calls flag with an unanswered "
            "batch pending — clearing and screening normally",
        )

    if not isinstance(last, AIMessage) or not last.tool_calls:
        return {"screener_route": PHASE1_SCREENER_ROUTE_PASS, "truncated_tool_calls": False}
    ...
    for tc in last.tool_calls:
        ...
        # Capability-probe exception: ``kubectl_read``'s ``debug`` creates an
        # ephemeral probe pod. It classifies as node/pod (NOT READONLY) because
        # ``kubectl debug`` creates a Pod — but ``kubectl_read`` self-gates the
        # debug ``--`` command AND every follow-up ``exec`` to read-only probes
        # (mutating inner is rejected at the tool layer), so a debug pod created
        # here can ONLY be used read-only, and Phase 1 exit cleans it up. Allow
    ...
    # ultimate backstop.
    if not enforcing and not has_context_reject:
        logger.warning(
