...
_STATE_FIELD_POLICY_LIST: tuple[StateFieldPolicy, ...] = (
    ...
    _p("plan_builder_prefetch_done", "planning", batch=False),
    ...
    _p("kube_connection_mode", "execution", durable=True),
    _p("host_name", "execution", durable=True),
    _p("ssh_host", "execution", durable=True),
    _p("ssh_user", "execution", durable=True),
    ...
    _p("_execute_text_stall_count", "execution", batch=0),
    _p("_injection_selfcheck_nudged", "execution", batch=False),
    ...
    # Reset per fault and per recover, like the loop counters beside it: the
    ...
    _p("replan_context_injected_attempt", "loop_control", batch=None, recover=None),
