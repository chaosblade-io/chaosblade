...
# Their final output is delivered via the result envelope: the postmortem
# markdown is generated by ``terminal_reports`` and attached to the result
# payload (``save_memory`` persists it), which the TUI renders as
# PostmortemSection. Streaming the tokens would render the same content
# twice — once as agent text, once as the card.
