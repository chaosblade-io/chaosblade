...
type Executor struct {
	...
	jvmExecutor spec.Executor
}

func NewWindowsExecutor() spec.Executor {
	return &Executor{
		executors:   model.GetAllOsExecutors(),
		jvmExecutor: jvm.NewExecutor(),
	}
}
...
func (e *Executor) Exec(uid string, ctx context.Context, model *spec.ExpModel) *spec.Response {
	...
	if executor == nil {
		return e.jvmExecutor.Exec(uid, ctx, model)
	}
	executor.SetChannel(channel.NewChannel())
